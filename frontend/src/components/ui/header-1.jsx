...
import { LuShoppingCart, LuLogOut, LuUser } from "react-icons/lu";
...
export function Header() {
  ...
  return (
    <header
      className={cn(
        "sticky top-0 z-50 w-full border-b border-transparent transition-transform duration-300",
        {
          "border-slate-200 bg-white/95 backdrop-blur-lg": scrolled,
    ...
    >
      <nav className="mx-auto flex h-16 w-full max-w-6xl items-center justify-between px-4">
        <Link to="/" className="rounded-md p-2 hover:bg-slate-100">
      ...
      <MobileMenu open={open} className="flex flex-col justify-between gap-4">
        <div className="grid gap-y-2">
          ...
          >
            <LuShoppingCart className="mr-2 size-4" />
            Cart ({totalItems})
          </Link>
        </div>
...
function MobileMenu({ open, children, className, ...props }) {
  ...
  return createPortal(
    <div
      id="mobile-menu"
      className={cn(
        "fixed top-16 right-0 bottom-0 left-0 z-40 flex flex-col overflow-hidden border-y border-slate-200 bg-white/95 backdrop-blur-lg md:hidden",
      )}
    >
      <div
        data-slot={open ? "open" : "closed"}
        className={cn(
          "size-full p-4 data-[slot=open]:animate-in data-[slot=open]:zoom-in-95",
          className,
        )}
        {...props}
      >
